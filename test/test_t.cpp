...
bool ThreadCondition1(void * aArg)
{
  lock_guard<mutex> lock(gMutex);
  -- gCount;
  gCond.notify_all();
...
bool ThreadCondition2(void * aArg)
{
  cout << " Wating..." << flush;
  lock_guard<mutex> lock(gMutex);
  while(gCount > 0)
...
int main()
{
  ...
  cout << endl << "PART II: Thread IDs" << endl;
  {
    ...
    threadt t1(ThreadIDs, 0);
    t1.join();
    threadt t2(ThreadIDs, 0);
    t2.join();
    threadt t3(ThreadIDs, 0);
    t3.join();
    threadt t4(ThreadIDs, 0);
    t4.join();
  }
  ...
  cout << endl << "PART IV: Mutex locking (100 threads x 10000 iterations)" << endl;
  {
    ...
    for(int i = 0; i < 100; ++ i)
      threadList.push_back(new threadt(ThreadLock, 0));
  ...
  cout << endl << "PART V: Fast mutex locking (100 threads x 10000 iterations)" << endl;
  {
    ...
    list<thread *> threadList;
    for(int i = 0; i < 100; ++ i)
      threadList.push_back(new threadt(ThreadLock2, 0));
  ...
  cout << endl << "PART VI: Atomic spin-lock (100 threads x 10000 iterations)" << endl;
  {
    ...
    list<thread *> threadList;
    for(int i = 0; i < 100; ++ i)
      threadList.push_back(new threadt(ThreadSpinLock, 0));
  ...
  cout << endl << "PART VII: Atomic variable (100 threads x 10000 iterations)" << endl;
  {
    ...
    list<thread *> threadList;
    for(int i = 0; i < 100; ++ i)
      threadList.push_back(new threadt(ThreadAtomicCount, 0));
  ...
  cout << endl << "PART VIII: Condition variable (40 + 1 threads)" << endl;
  {
    ...
    threadt t1(ThreadCondition2, 0);

    // Start a bunch of child threads (these will decrease gCount by 1 when they
    // finish)
    list<thread *> threadList;
    for(int i = 0; i < 40; ++ i)
      threadList.push_back(new threadt(ThreadCondition1, 0));
  ...
  cout << endl << "PART IX: Yield (40 + 1 threads)" << endl;
  {
    ...
    list<thread *> threadList;
    for(int i = 0; i < 40; ++ i)
      threadList.push_back(new threadt(ThreadYield, 0));
